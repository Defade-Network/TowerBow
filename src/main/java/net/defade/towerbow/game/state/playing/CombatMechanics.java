...
public class CombatMechanics {
    ...
    private final EventNode<EntityInstanceEvent> gameMechanicsNode;

    private CombatMechanics() {
        this.gameMechanicsNode = createPvPNode();
        disableArrowSpread();
        disableFriendlyFire();
    ...
    private void disableArrowSpread() {
        gameMechanicsNode.addListener(EntityShootEvent.class, entityShootEvent -> {
            entityShootEvent.setSpread(0); // Disable arrow randomness
        });
    }

    private void disableFriendlyFire() {
        gameMechanicsNode.addListener(EntityDamageEvent.class, entityDamageEvent -> {
    ...
    public static EventNode<EntityInstanceEvent> create() {
        return new CombatMechanics().gameMechanicsNode;
    }
}
